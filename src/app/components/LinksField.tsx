...
export function LinksEditor( { links, onChange }: { links: ItemLink[]; onChange: ( next: ItemLink[] ) => void } ) {
  // Stable per-row keys so removing a middle row keeps each remaining row's
  // identity (and input focus) intact — plain index keys would shift them.
  // Ids are managed only in handlers (never read/written during render). Mount
  // this editor with a key per item (see DetailModal) so ids re-init per item.
  const [ ids, setIds ] = useState<number[]>( () => links.map( ( _, i ) => i ) );
  const [ nextId, setNextId ] = useState( links.length );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      { links.map( ( link, i ) => (
        <div key={ ids[i] ?? i } style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
          ...
          <button type="button" onClick={ () => remove( i ) } title="Remove link" aria-label="Remove link" style={{ background: 'none', border: 'none', cursor: 'pointer', color: C.mutedFg, display: 'flex', flexShrink: 0 }}>
            <X size={ 16 } />
          </button>
        </div>
      ) ) }
