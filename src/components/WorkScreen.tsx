...
/**
 * The work screen: one site's work, in whichever view is chosen.
 *
 * Scoped to one site and never to more than one, because that is how the work
 * itself is scoped (ARCH-3): an item belongs to a site, and a board showing two
 * sites at once would be showing two different pieces of work in the same
 * column with no way to tell them apart. The request queue (#131) is the one
 * studio screen that does span clients, and it is a separate screen for exactly
 * this reason — nothing here has to bend to accommodate it.
 *
 * Everything this can be other than a board — loading, empty, broken, not yours
    ...
    <>
      ...
      { 'ready' === shell && 0 < sites.length && 'ready' === board && (
        <>
          ...
          { 'calendar' === view && (
            <CalendarView items={ items } onOpen={ ( item ) => setOpenId( item.id ) } onOpenId={ setOpenId } />
          ) }
