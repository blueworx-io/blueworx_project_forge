...
`fonts/` next to it. Also copy `design-system.php` to `assets/blueworx-admin-design.php`, in the
same folder as the stylesheet — it works out its own asset URL from where it sits, so it has to
live beside it — and copy `assets/icons/lucide-icons.js` to `assets/blueworx-admin-icons.js`.
Never hand-write brand CSS that duplicates what is already in there.
