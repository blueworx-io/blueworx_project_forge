	...
	public static function seed() {
		...
		$f3_id = self::insert_post(
			...
			array(
				...
				'_forge_image_urls'         => array( 'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?w=800' ),
			)
		);

		$f4_id = self::insert_post(
			'forge_feature',
		...
		update_post_meta( $f1_id, '_forge_subitem_ids', array( (string) $s1_id, (string) $s2_id ) );

		// ----------------------------------------------------------------
		// Bugs
		// ----------------------------------------------------------------
		$b1_id = self::insert_post(
			'forge_bug',
		...
		$b2_id = self::insert_post(
			'forge_bug',
			'Memory leak in data grid',
		...
		$fb2_id = self::insert_post(
			...
			array(
				...
				'_forge_urls'              => array( 'https://feedback.example.com/mobile-ux' ),
			)
		);

		$fb3_id = self::insert_post(
			'forge_feedback',
			'Dark mode color contrast needs improvement',
		...
		update_post_meta( $r1_id, '_forge_linked_feature_ids', array( (string) $f1_id, (string) $f2_id, (string) $f3_id ) );
		update_post_meta( $r1_id, '_forge_linked_bug_ids', array( (string) $b1_id ) );
		update_post_meta( $r1_id, '_forge_linked_feedback_ids', array( (string) $fb3_id ) );

		update_post_meta( $r2_id, '_forge_linked_feature_ids', array( (string) $f4_id, (string) $f6_id ) );
		update_post_meta( $r2_id, '_forge_linked_bug_ids', array( (string) $b3_id ) );
		update_post_meta( $r2_id, '_forge_linked_feedback_ids', array() );
